...
if [[ -s ${FQ1} ]]; then
	...
	if [[ ${GTF} != "NA" ]]; then
		...
		echo "tophat2 $ALN -o $BAMDIR --transcriptome-index $TRPREF $SEQPREF ${FQ1}" >> ${alignScript}
	    else
		echo "tophat2 $ALN -o $BAMDIR -r 200 --mate-std-dev 20 --transcriptome-index $TRPREF $SEQPREF ${FQ1} ${FQ2}" >>${alignScript} 
	    fi
	else
	    #no gtf file here
	    if [[ $FQ2 == "NA" ]]; then #SE analysis
		echo "tophat2 $ALN -o $BAMDIR $SEQPREF ${FQ1}" >>${alignScript} 
	    else
		echo "tophat2 $ALN -o $BAMDIR -r 200 --mate-std-dev 20 $SEQPREF ${FQ1} ${FQ2}" >>${alignScript}
	    fi
	fi
    fi

    if [[ $CLEAN -eq 1 || ! -f ${BAMDIR}/${MPREF}.bam ]]; then
	echo "samtools view -H ${BAMDIR}/accepted_hits.bam | sed 's/SO:coordinate/SO:unsorted/' > ${BAMDIR}/header.sam" >>${alignScript}
	echo "samtools cat -h ${BAMDIR}/header.sam ${BAMDIR}/accepted_hits.bam ${BAMDIR}/unmapped.bam | samtools view -b -F0x100 - | samtools sort -n -m 2000000000 - ${BAMDIR}/${MPREF}" >>${alignScript}
	echo "rm ${BAMDIR}/accepted_hits.bam ${BAMDIR}/*bed ${BAMDIR}/unmapped.bam" >>${alignScript}
    fi
fi
