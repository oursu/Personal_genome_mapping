...
if [[ -s ${FQ1} && ( -s ${FQ2} || $FQ2 == "NA" ) ]]; then
    ...
    format=`/home/oursu/devtools/Python-2.7.6/python /srv/gsfs0/projects/kundaje/users/oursu/code/personalGenomeAlignment/new_ase_code/ase/python/checkFastq_2014-03-03.py --\
fastq $FQ1`
    echo ${format}
...
if [[ -s ${BAMDIR}/${SAMPLE}_1.sai && ( $FQ2 == "NA" || -s ${BAMDIR}/${SAMPLE}_2.sai ) ]]; then
    if [[ $CLEAN -eq 1 || ! -f ${BAMDIR}/${SAMPLE}.bam ]]; then
        ...
        if [[ $FQ2 == "NA" ]]; then
            ${BWA_PATH} samse -r $head $SEQPREF ${BAMDIR}/${SAMPLE}_1.sai ${FQ1} | sed -r 's/_[12]:N:0:[ACGT]+//' | ${SAMTOOLS_PATH} view -Sbh -t ${SEQPREF}.fa.fai - | ${SAM\
TOOLS_PATH} sort $SORTOPT -m 2000000000 - ${BAMDIR}/${SAMPLE}
        else
            # Notice the sort by name here...                                                                                                                                 
            ${BWA_PATH} sampe -r $head $SEQPREF ${BAMDIR}/${SAMPLE}_1.sai ${BAMDIR}/${SAMPLE}_2.sai ${FQ1} ${FQ2} | sed -r 's/_[12]:N:0:[ACGT]+//' | ${SAMTOOLS_PATH} view -S\
bh -t ${SEQPREF}.fa.fai - | ${SAMTOOLS_PATH} sort $SORTOPT -m 2000000000 - ${BAMDIR}/${SAMPLE}
        fi
        if [[ $SORTPOS -eq 1 ]]; then
